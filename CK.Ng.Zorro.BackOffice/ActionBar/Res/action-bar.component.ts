...
import { Action, ActionBarContent } from './models/action-bar.model';
...
export class ActionBarComponent<T> {
    ...
    shouldShow( action: Action<T> ): boolean {
        return action.shouldBeDisplayed( this.selectedItems() );
    }

    buttonClicked( action: Action<T> ): void {
        action.execute( this.selectedItems() );
    }

    getButtonClass( action: Action<T> ): string {
        if ( action.class ) {
    ...
    isDisabled( action: Action<T> ): boolean {
